...
namespace elastic
{
	...
	{
	protected:
		binary_oarchive_impl(std::basic_streambuf<_Elem, _Traits>& os, uint32_t flags)
			: basic_binary_oprimitive<_Archive, _Elem, _Traits>(os, (flags & static_cast<int>(active_flags::no_header)))
			, basic_binary_oarchive<_Archive>(flags)
		{}

		binary_oarchive_impl(std::basic_ostream<_Elem,_Traits>& os, uint32_t flags)
			: basic_binary_oprimitive<_Archive, _Elem, _Traits>(*os.rdbuf(), (flags & static_cast<int>(active_flags::no_header)))
			, basic_binary_oarchive<_Archive>(flags)
		{}
	...
	public:
		...
		{
			if ((flags & static_cast<int>(active_flags::no_header)) != 0)
				return;
